...
#    print('processing file {} of {}'.format(inwb, len(nwb_paths)))
    ...
    unitchannels = unitchannels.sort_values('probe_vertical_position', ascending=False)
    ...
    for idur, duration in enumerate(durations):
        for il, level in enumerate(levels):
            ...
            for iu, unit in unitchannels.iterrows():
                ...
                if duration<0.1:
                    resp = makePSTH_numba(sts, opto_times-0.1, 0.2, binSize=0.001, convolution_kernel=0.002)[0]
                    resp = resp - np.mean(resp[:99])
                
                if duration>0.1:
                    resp = makePSTH_numba(sts, opto_times-0.25, 1.5, binSize=0.001, convolution_kernel=0.01)[0]
            ...
            im = ax[il][idur].imshow(all_resp, origin='lower', interpolation='none', aspect='auto')
            min_clim_val = -5
            max_clim_val = 50
            im.set_clim([min_clim_val, max_clim_val])    
                
        
    fig.savefig(os.path.join(nwb_validation_dir, 'optotagging', str(sessionID)+'.png'))
    plt.close('all')
#
#pool = Pool()
...
#    print('processing file {} of {}'.format(inwb, len(nwb_paths)))
    ...
    for ip, probeid in enumerate(probes):
        probe_unitchannels = unitchannels.loc[unitchannels['probe_id']==probeid]
    ...
    fig.savefig(os.path.join(nwb_validation_dir, 'all_spike_hists', str(sessionID)+'.png'))
    plt.close('all')
    return ash_dict
...
for inwb, nwb_path in enumerate(nwb_paths):
    ...
    passive_change_times = stimulus_presentations.loc[~stimulus_presentations['active']&
                                              stimulus_presentations['is_change']&
                                              (~np.isin(stimulus_presentations['image_name'], ['im111_r', 'im083_r']))]
    
    passive_change_times = passive_change_times['start_time'].values
    
    omission_times = stimulus_presentations.loc[stimulus_presentations['active']&
                                              stimulus_presentations['omitted']]['start_time'].values
    
    passive_omission_times = stimulus_presentations.loc[~stimulus_presentations['active']&
                                              stimulus_presentations['omitted']]['start_time'].values
...
#    shared_change_times = shared_change_times['start_time'].values
    ...
    flashes = stimulus_presentations[stimulus_presentations['stimulus_block']==4]
    flash_times = flashes[flashes['color']==1].start_time.values
    
    gabors = stimulus_presentations[stimulus_presentations['stimulus_block']==2]   
    gabor_times = gabors.groupby(['position_x', 'position_y'])['start_time'].agg(list)
    ...
    for iu, unit in unitchannels.iterrows():
        ...
        if 'VIS' in area:
            ...
            sts = spike_times[iu]
            cr = makePSTH_numba(sts, change_times-1, 2, binSize=0.001, convolution_kernel=0.005)
            pcr = makePSTH_numba(sts, passive_change_times-1, 2, binSize=0.001, convolution_kernel=0.005)
            aor = makePSTH_numba(sts, omission_times-1, 2, binSize=0.001, convolution_kernel=0.005)
            por = makePSTH_numba(sts, passive_omission_times-1, 2, binSize=0.001, convolution_kernel=0.005)
            flr = makePSTH_numba(sts, flash_times-0.5, 1, binSize=0.001, convolution_kernel=0.005)
            gr = np.array([makePSTH_numba(sts, np.array(gabor_times[x][y])-0.5, 1, binSize=0.001, convolution_kernel=0.005)[0] for x in gabors['position_x'].unique() for y in gabors['position_y'].unique()])
...
for sessionID in data_df.session.unique():
    ...
    for visarea in sdf.area.unique():
        ...
        for plot in ['active_change_response', 'passive_change_response', 
                     'omission_response', 'passive_omission_response']:
        ...
        ax.legend(['active_change_response', 'passive_change_response', 
                     'omission_response', 'passive_omission_response'])
        fig.savefig(os.path.join(nwb_validation_dir, 'image_vis_response', str(sessionID)+'_'+visarea+'.png'))
        
        ax_inset.legend(['active_change_response', 'passive_change_response', 
                     'omission_response', 'passive_omission_response'])
        fig_inset.savefig(os.path.join(nwb_validation_dir, 'image_vis_response', str(sessionID)+'_'+visarea+'_latency.png'))
        
        
        
        axrf.legend(['rf_response', 'flash_response'])
        figrf.savefig(os.path.join(nwb_validation_dir, 'rf_vis_response', str(sessionID)+'_'+visarea+'.png'))
