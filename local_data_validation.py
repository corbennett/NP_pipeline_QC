...
def upload_ready_validation(local_dir, schema):
    ...
    for name, file in schema.items():
        ...
        (meets_criterion, file_size, min_size) = validate_schema_entry_size(local_dir, file)
        ...
        if not exists:
            ...
            report['Errors'].append('File {} does not meet size criterion'.format(name))
...
def validate_schema_entry_size(local_dir, schema_file):
    ...
    if not min_size:
        ...
        file_size = get_file_size(glob_file(local_dir, schema_file['rel_path']))
