...
def get_opt_data_dir(mouse_id, save_base):
    ...
    if d is None:
        print('Could not find OPT data directory for {} in {}'.format(mouse_id, save_base))
...
def post_to_lims(mouse_id, specimen_id, project_id, operator_id, date):
    ...
    payload = {'name' : date+'_'+mouse_id+'_opt',
...
def make_trigger_file(platform_save_dir, opt_lims_id, lims_incoming_dir):
    ...
    with open(trigger_file, 'w+') as triggerfile:
        ...
        L2 = r"location: '{}'".format(lims_incoming_dir + '/' + str(opt_lims_id) + '_platform.json') + "\n"
...
def make_platform_file(platform_save_dir, mouse_id, opt_lims_id, lims_incoming_dir, user_name, rig, acquisition_date):
    platform_file = os.path.join(platform_save_dir, str(opt_lims_id) + '_platform.json')
...
def transfer_data(mouse_id, opt_data_dir, lims_incoming_dir):
    ...
    print('Copied {} to {} with return code {}'.format(opt_data_dir, dest_dir, P))


### copy trigger and platform files ###
def transfer_trigger_platform(trigger_dir, lims_incoming_dir, trigger_file, platform_file):
    trigger_dir_norm = '\\' + os.path.normpath(trigger_dir)
    lims_incoming_norm = '\\'+os.path.normpath(lims_incoming_dir)
    shutil.copyfile(trigger_file, os.path.join(trigger_dir_norm, os.path.basename(trigger_file)))
    shutil.copyfile(platform_file, os.path.join(lims_incoming_norm, os.path.basename(platform_file)))
...
#    rig = 'OPT.0'
    ...
    specimen_id, project_id, operator_id = query_lims_for_ids(mouse_id, project_name, user_name)
    ...
    trigger_file = make_trigger_file(platform_save_dir, opt_lims_id, lims_incoming_dir)
    platform_file = make_platform_file(platform_save_dir, mouse_id, opt_lims_id, lims_incoming_dir, user_name, rig, acquisition_date)
    
    transfer_data(mouse_id, opt_data_dir, lims_incoming_dir)
    transfer_trigger_platform(trigger_dir, lims_incoming_dir, trigger_file, platform_file)
    
if __name__ == "__main__":
    
    parser = argparse.ArgumentParser()
    parser.add_argument("mouse_id",
                    help="mouse ID (6 digit labtracks id)")
    ...
    parser.add_argument("-u", "--user", help="Operator name (allen login)", default="corbettb")
    
    parser.add_argument("-r", "--rig", help="OPT rig on which data was collected (OPT.0 or OPT.1)",
                        default = "OPT.0")
